...
async function loadProjects() {
  ...
  if (currentFeatured !== null)
    url.searchParams.set("featured", currentFeatured);
...
function renderProjects(projects) {
  ...
  tbody.innerHTML = projects
    .map((p) => {
      const desc =
        escapeHtml(p.description.substring(0, 90)) +
      ...
      <tr>
        ...
        <td class="cell-actions">
          <button class="table-btn js-edit-btn admin-action-btn" data-id="${p.id}" data-title="${escapeHtml(p.title)}">
            <i class="fa-solid fa-pen" style="font-size:11px"></i> Edit
          </button>
          <button class="table-btn table-btn--danger js-delete-btn admin-action-btn" data-id="${p.id}" data-title="${escapeHtml(p.title)}">
...
async function confirmDelete() {
  ...
  try {
    const res = await fetch(`/api/admin/projects/${pendingDeleteId}`, {
      method: "DELETE",
    });
    const data = await res.json();
...
function closeDrawer() {
  ...
  document
    ...
    .classList.remove("user-drawer--open");
...
function collectPayload() {
  return {
    ...
    github_url:
      document.getElementById("field-github-url").value.trim() || null,
...
async function saveProject() {
  ...
  const url = isEdit
    ? `/api/admin/projects/${drawerEditId}`
    : "/api/admin/projects";
...
// Open create drawer
document
  .getElementById("open-create-drawer")
  .addEventListener("click", () => openDrawer("create"));
...
document.addEventListener("keydown", (e) => {
  ...
  if (
    document
      .getElementById("project-drawer")
      .classList.contains("user-drawer--open")
  ) {
    closeDrawer();
  } else {
    closeDeleteModal();
...
document.addEventListener("click", (e) => {
  ...
  if (editBtn) {
    openEditDrawer(editBtn.dataset.id);
    return;
  }
...
document.getElementById("featured-filter").addEventListener("change", (e) => {
  ...
  currentFeatured =
    val === "featured" ? true : val === "not-featured" ? false : null;
  currentPage = 1;
  loadProjects();
});
...
function applyViewerRestrictions() {
  ...
  const newProjectBtn = document.getElementById("open-create-drawer");
  if (newProjectBtn) newProjectBtn.style.display = "none";
  if (newProjectBtn) newProjectBtn.style.display = "none";

  // Table rows — Edit and Delete buttons
  document
    .querySelectorAll(".js-edit-btn, .js-delete-btn")
